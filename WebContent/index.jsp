...
<script type="text/javascript">
	...
	function file_json2(result) {
		...
		$.each(result.extend.pageInfo.list, function(index, current) {
			...
			var fileName = $("<a href=''></a>").append(current.fileName).prop(
					"href", "file/fileDetail?fileId=" + current.fileId);
			var classUp = $("<div class='up'></div>").append(fileName);
			var nameAndTime = $("<div class='down'></div>").append(
					"上传者:" + current.user.userName + "&emsp;上传时间:"
							+ Transdate(current.uploadDate));
			var classCs = $("<div class='cs'></div>").append(classUp).append(
					nameAndTime);
			var score = $("<div class='arr'></div>").append(
					"积分<span>" + current.score + "</span>");
			var pro = $("<div class='pro'></div>");
			pro.append(img).append(classCs).append(score).appendTo("#main");
		})
	...
	function page_json(result) {
		...
		$("#panigation").append(firstPage).append("&emsp;").append(lastPage)
				.append("&emsp;").append(nextPage).append("&emsp;").append(
						endPage).append("&emsp;");
		$("#panigation").append(
				"共有" + result.extend.totalPage + "页,当前是第"
						+ result.extend.currentPage + "页");
	}
	//分页查询
	...
	function search_page_json(result) {
		...
		$("#panigation").append(firstPage).append("&emsp;").append(lastPage)
				.append("&emsp;").append(nextPage).append("&emsp;").append(
						endPage).append("&emsp;");
		$("#panigation").append(
				"一共有" + result.extend.totalCount + "条数据,当前是第"
						+ result.extend.currentPage + "页");
	}
	//登陆验证
	function check() {
...
<body>
		...
		<div class="newModWin">
			...
			<div class="logWin">
				<form action="user/login" method="post" onsubmit="return check()">
					...
					<input type="text" id="userName" class="inp user" name="userName"
						autocomplete="off" placeholder="请输入用户名" /> <input type="password"
						id="password" class="inp pass" name="password" autocomplete="off"
						placeholder="请输入密码" /> <a href="findpass.jsp" class="find_pass">忘记密码，立即找回</a>
					<input type="submit" class="su_btn" value="登录" /> <a
						href="regist.jsp" class="reg">注册</a>
				</form>
			</div>
		</div>
	</div>
	...
	<div id="nav">
		...
		<div id="logon">
			<div class="cons" id="cons">欢迎光临CSDN！</div>
			<div class="opers">
